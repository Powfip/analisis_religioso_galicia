...
def limpiar_dataframe(ruta, header, fila_eliminar, renombrar_primera_col=True):
    """
    Limpiar un archivo Excel eliminando filas sobrantes y reestablecer cabeceras.
    Args:
        ruta (str): ruta al archivo Excel
        header (list): número de fila a usar como cabecera
        fila_eliminar (int o list): número(s) de fila(s) a eliminar después
        renombrar_primera_col (bool): si True, renombra la primera columna a 'Perfil
    Returns:
    ...
    if pd.isna(df.columns[0] and renombrar_primera_col):
        df.columns[0] = 'Perfil' + list(df.columns[1:])
    ...
    df.columns = [col.replace('Unnamed: 0', 'Perfil').strip() if isinstance(col, str) else 'son_nombre' for col in df.columns]
    return df
